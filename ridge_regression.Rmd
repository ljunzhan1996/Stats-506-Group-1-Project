...
#Introduction
Among ordinary OLS regression, it is easy to cause problems, for example, imprecise 
estimate of coefficients, measurement error sensitivity and numerical instability 
when predictors are highly correlated. To deal with mutiple collinearity of 
predictors, we could choose to use ridge regression rather.
...
the pseudo inverse of A matrix. Then we can compute the pseudo inverse of the 
matrix of our problem and get the answer of ridge coefficients \beta.
...
\]
Now we briefly introduce how to compute the regression coeficients. In the 
following part, we'll place more efforts in $\lambda$ selection's work, which is 
equally an important part in ridge regression.
...
very high collinearity, where adjacent channels are almost perfectly correlated. 
...
Below is a matplot of absorbances(y) agains 100 channel spectrums 
...
####Group
To split dataset into 5 groups, a new column "gp" was added with values 0, 1, 2, 3, 4.  
...
automatically using the method proposed by Cule et at(2012).
```
library("ridge")
```
...
total variances in the predictors due to collinearity. Since it exceeded the 90% 
...
#### $*$ **Our Solution**
In case of almost perfect multicollinearity, we propose to calculate Singular value 
decomposition separately and specify your own "nPCs."

Meanwhile, you can choose your own threshold and reflect it by setting "nPCs" value.

Instead of relying on the 90% threshold set within the function, more flexibility is 
granted for users in this way. 

#### $*$ Fit linear ridge regression with nPCs = 1

Based on training data, we fit linear lasso regression using function "linearRidge", 
...
our validation data. The RMSE calculated based on prediction was about 10.
```
...
explains more than **99.99%** variances in the predictors. 
...
extreme scenarios, where predictors are almost perfectly multicollinearity. Under such 
...
Since the data are clean, we don't need to do much work about the data cleaning. Let's 
...
to transform the coefficient $\beta$ in to 
...
the related coefficient. Since in the meatspec data we use has 100 predictors, which 
...
$\lambda_{Stata}$=5e-10. But explore the reason for such difference, we find it arises 
...
set cross-validation's partitions before we loop for different $\lambda$ value. The general 
trend of plot doesn't change but with the fulctuation in $\lambda$. We can see that 
...
If we retrict the range for lamda selection in loop [0,1e-10,3e-8] instead of the loop 
...
fluctuation could not cause much problem.

#####(iii)Softwares' relatively different ways to compute
Another reason for difference in selecting $\lambda$ is Matlab, R, Stata use somewhat 
different ways for such selection. In **Matlab**, there is no function can directly get 
the $\lambda$, so I combine the loop for $\lambda$ and cross-validation function to get 
the $\lambda$ with least MSE in validtaion set. In **R**, Ridge package combine the PCA 
...
Run the code above, we can get the MSE=7.78, which change not much from the validation 
...
model. It is a kind of method to improve the testing results .This is caused by the 
differences between programming approaches. Other two samples show a good result even 
without this step.
...
We used different method in seleting $\lambda$. **Matlab** does not provide 
any built in function thus we wrote our own cross-validation funtion and 
selected $\lambda$ based on MSE. Package RIDGE was used in **R**, which 
...
value of $\lambda$ is very small. We believe different precision level set 
within each software is one of the reason for difference in our estimation.
